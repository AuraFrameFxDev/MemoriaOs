...
class BuildScriptsValidationV2Test {
    ...
    @Test
    fun `kotlin jvm toolchain set to 24 and kotlinOptions target set to 24 if present`() {
        val content = buildFile.readText()

        // kotlinOptions target (string or without quotes depending on style)
        val hasJvmTarget24 =
            content.contains(Regex("""kotlinOptions\s*\{\s*[^\}]*jvmTarget\s*=\s*["']?24["']?""", RegexOption.DOT_MATCHES_ALL)) ||
            content.contains(Regex("""compilerOptions\s*\{\s*[^\}]*jvmTarget\s*=\s*JvmTarget\.JVM_24""", RegexOption.DOT_MATCHES_ALL))
        assertTrue("kotlinOptions or compilerOptions jvmTarget should be 24", hasJvmTarget24)
    }
    ...
    fun `compose build features enabled and compose compiler extension configured`() {
        ...
        val hasComposeBuildFeatures = content.contains(Regex("""buildFeatures\s*\{\s*[^}]*compose\s*=\s*true""", RegexOption.DOT_MATCHES_ALL))
    ...
    fun `android namespace and sdk versions are defined`() {
        val content = buildFile.readText()
        val hasNamespace = content.contains(Regex("""android\s*\{\s*[^}]*namespace\s*=""", RegexOption.DOT_MATCHES_ALL))
        ...
        val hasDefaultConfigMinSdk = content.contains(Regex("""defaultConfig\s*\{\s*[^}]*minSdk\s*=\s*\d+""", RegexOption.DOT_MATCHES_ALL))
        assertTrue("defaultConfig.minSdk should be defined", hasDefaultConfigMinSdk)

        val hasDefaultConfigTargetSdk = content.contains(Regex("""defaultConfig\s*\{\s*[^}]*targetSdk\s*=\s*\d+""", RegexOption.DOT_MATCHES_ALL))
    ...
    fun `serialization and compose plugins imply matching dependencies present`() {
        ...
        val hasSerializationPlugin = content.contains("""id("org.jetbrains.kotlin.plugin.serialization")""")
        if (hasSerializationPlugin) {
            val hasSerializationDep = content.contains(Regex("""(implementation|api)\(\s*["']org\.jetbrains\.kotlinx:kotlinx-serialization-(json|core).*["']\s*\)"""))
            assertTrue("kotlinx-serialization dependency should be present when serialization plugin is applied", hasSerializationDep)
        }
        ...
        if (hasComposePlugin) {
            val hasComposeDependency = content.contains(Regex("""(implementation|api)\(\s*["']androidx\.compose\.[^"']+["']\s*\)"""))
                || content.contains(Regex("""(implementation|api)\(\s*libs\.androidx\.compose\.[^)]+"""))
            assertTrue("At least one Compose dependency should be present when compose plugin is applied", hasComposeDependency)
        }
    }
    ...
    fun `ksp plugin is applied and used consistently for processors`() {
        val content = buildFile.readText()
        assertTrue("KSP plugin must be applied", content.contains("""id("com.google.devtools.ksp")"""))
        ...
        if (hasKapt) {
            val hasKaptJustification = content.contains(Regex("""//\s*kapt(?:\s+required|:|\s+reason)""", RegexOption.IGNORE_CASE))
            assertTrue("kapt present without justification comment; prefer KSP or justify why kapt is required", hasKaptJustification)
        }
    ...
    fun `preBuild dependsOn statements are inside the preBuild block`() {
        val content = buildFile.readText()
        val preBuildBlockRegex = Regex("""tasks\.named\("preBuild"\)\s*\{\s*(.*?)\s*\}""", RegexOption.DOT_MATCHES_ALL)
        ...
        val hasOpenApiInside = block.contains("""dependsOn(":openApiGenerate")""") || block.contains("""dependsOn("openApiGenerate")""")
    ...
    fun `packaging excludes are comprehensive and use set+= pattern`() {
        ...
        val usesSetStyle = content.contains(Regex("""resources\s*\{\s*[^}]*excludes\s*\+=\s*setOf\(""", RegexOption.DOT_MATCHES_ALL))
        assertTrue("Packaging.resources should use 'excludes += setOf(...)' style", usesSetStyle)

        // Core excludes
        assertTrue("Excludes should include META-INF AL2.0 and LGPL2.1", content.contains("/META-INF/{AL2.0,LGPL2.1}"))
        assertTrue("Excludes should include META-INF/DEPENDENCIES", content.contains("/META-INF/DEPENDENCIES"))
    ...
    fun `jniLibs pickFirst entries are declared only once and legacy packaging disabled`() {
        ...
        val pickFirstPattern = Regex("""pickFirsts?\s*\+\=\s*setOf\([^)]*\)|pickFirst\(\s*["'][^"']+["']\s*\)""")
        ...
        assertTrue("Pick first libc++_shared.so must be present", content.contains("**/libc++_shared.so"))
    ...
    fun `release buildType has minify and proguard rules configured`() {
        ...
        val releaseBlock = Regex("""buildTypes\s*\{\s*[^}]*release\s*\{\s*(.*?)\s*\}""", RegexOption.DOT_MATCHES_ALL)
            .find(content)?.groupValues?.getOrNull(1) ?: ""
        assertTrue("release buildType should be configured", releaseBlock.isNotBlank())

        val hasMinify = releaseBlock.contains(Regex("""\b(minifyEnabled|isMinifyEnabled)\s*=\s*(true|false)"""))
        ...
        val hasProguardFiles = releaseBlock.contains(Regex("""proguardFiles\s*\(""")) || releaseBlock.contains(Regex("""proguardFile\s*\("""))
