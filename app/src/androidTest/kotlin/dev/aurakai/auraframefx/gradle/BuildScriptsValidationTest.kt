...
class BuildScriptsValidationTest {
    ...
    fun `build script contains required plugin declarations`() {
        ...
        assertTrue("Should contain Android application plugin", 
        ...
        assertTrue("Should contain KSP plugin", 
        ...
        assertTrue("Should contain Kotlin serialization plugin", 
            content.contains("alias(libs.plugins.kotlin.serialization)"))
        assertTrue("Should contain Google services plugin", 
            content.contains("alias(libs.plugins.google.services)"))
        assertTrue("Should contain OpenAPI generator plugin", 
            content.contains("alias(libs.plugins.openapi.generator)"))
        assertTrue("Should contain Firebase Crashlytics plugin", 
            content.contains("alias(libs.plugins.firebase.crashlytics)"))
        assertTrue("Should contain Firebase Performance plugin", 
            content.contains("alias(libs.plugins.firebase.perf)"))
        assertTrue("Should contain Compose plugin", 
    ...
    fun `android configuration has required basic settings`() {
        ...
        assertTrue("Should set namespace", content.contains("namespace = \"dev.aurakai.auraframefx\""))
    ...
    fun `defaultConfig has proper application settings`() {
        ...
        assertTrue("Should set applicationId", 
        ...
        assertTrue("Should set test runner", 
    ...
    fun `packaging configuration excludes unwanted files`() {
        ...
        assertTrue("Should keep debug symbols", 
    ...
    fun `release build type configuration is valid`() {
        ...
        assertTrue("Release should disable minification for debugging", 
            content.contains("isMinifyEnabled = false"))
        assertTrue("Should include default proguard file", 
            content.contains("getDefaultProguardFile(\"proguard-android-optimize.txt\")"))
        assertTrue("Should include custom proguard rules", 
    ...
    fun `kotlin compiler options are properly configured`() {
        ...
        assertTrue("Should enable JVM default methods", 
            content.contains("\"-Xjvm-default=all\""))
        assertTrue("Should enable context receivers", 
            content.contains("\"-Xcontext-receivers\""))
        assertTrue("Should opt-in to RequiresOptIn", 
    ...
    fun `cmake configuration is valid`() {
        ...
        assertTrue("Should specify CMakeLists.txt path", 
            content.contains("path = file(\"src/main/cpp/CMakeLists.txt\")"))
        assertTrue("Should use cmake version from root project", 
    ...
    fun `lint configuration is comprehensive`() {
        ...
        assertTrue("Should have lint baseline", 
            content.contains("baseline = file(\"lint-baseline.xml\")"))
        assertTrue("Should check dependencies", 
            content.contains("checkDependencies = true"))
        assertTrue("Should have lint config", 
            content.contains("lintConfig = file(\"lint.xml\")"))
        assertTrue("Should treat warnings as errors", 
            content.contains("warningsAsErrors = true"))
        assertTrue("Should abort on error", 
            content.contains("abortOnError = true"))
        assertTrue("Should check release builds", 
            content.contains("checkReleaseBuilds = true"))
        assertTrue("Should check generated sources", 
    ...
    fun `ksp room schema location is configured`() {
        ...
        assertTrue("Should configure Room schema location", 
    ...
    fun `source sets include generated sources`() {
        ...
        assertTrue("Should include generated kotlin sources", 
    ...
    fun `force resolution strategy is configured`() {
        ...
        assertTrue("Should prefer project modules", 
            content.contains("preferProjectModules()"))
        assertTrue("Should force kotlin stdlib version", 
            content.contains("\"org.jetbrains.kotlin:kotlin-stdlib:\${libs.versions.kotlin.get()}\""))
        assertTrue("Should force kotlin reflect version", 
    ...
    fun `core android dependencies are included`() {
        ...
        assertTrue("Should include core library desugaring", 
            content.contains("coreLibraryDesugaring(libs.desugarJdkLibs)"))
        assertTrue("Should include AndroidX core", 
            content.contains("implementation(libs.androidxCoreKtx)"))
        assertTrue("Should include AppCompat", 
    ...
    fun `lifecycle dependencies are included`() {
        ...
        assertTrue("Should include lifecycle runtime", 
            content.contains("implementation(libs.androidxLifecycleRuntimeKtx)"))
        assertTrue("Should include lifecycle viewmodel compose", 
            content.contains("implementation(libs.lifecycleViewmodelCompose)"))
        assertTrue("Should include lifecycle viewmodel", 
            content.contains("implementation(libs.androidxLifecycleViewmodelKtx)"))
        assertTrue("Should include lifecycle runtime compose", 
    ...
    fun `dagger hilt dependencies are complete`() {
        ...
        assertTrue("Should include Hilt Android", 
            content.contains("implementation(libs.hiltAndroid)"))
        assertTrue("Should include Hilt compiler with KSP", 
            content.contains("ksp(libs.hiltCompiler)"))
        assertTrue("Should include Hilt navigation compose", 
            content.contains("implementation(libs.hiltNavigationCompose)"))
        assertTrue("Should include Hilt work", 
    ...
    fun `room database dependencies are configured`() {
        ...
        assertTrue("Should include Room runtime", 
            content.contains("implementation(libs.androidxRoomRuntime)"))
        assertTrue("Should include Room KTX", 
            content.contains("implementation(libs.androidxRoomKtx)"))
        assertTrue("Should include Room compiler with KSP", 
    ...
    fun `network dependencies are configured`() {
        ...
        assertTrue("Should include Retrofit", 
            content.contains("implementation(libs.retrofit)"))
        assertTrue("Should include Gson converter", 
            content.contains("implementation(libs.converterGson)"))
        assertTrue("Should include OkHttp", 
            content.contains("implementation(libs.okhttp)"))
        assertTrue("Should include logging interceptor", 
            content.contains("implementation(libs.okhttpLoggingInterceptor)"))
        assertTrue("Should include Kotlin serialization converter", 
    ...
    fun `unit testing dependencies are complete`() {
        ...
        assertTrue("Should include MockK agent", 
    ...
    fun `android testing dependencies are complete`() {
        ...
        assertTrue("Should include AndroidX JUnit extension", 
            content.contains("androidTestImplementation(libs.androidxTestExtJunit)"))
        assertTrue("Should include Espresso core", 
            content.contains("androidTestImplementation(libs.espressoCore)"))
        assertTrue("Should include Compose BOM for tests", 
        ...
        assertTrue("Should include MockK Android", 
            content.contains("androidTestImplementation(libs.mockkAndroid)"))
        assertTrue("Should include Hilt testing", 
            content.contains("androidTestImplementation(libs.hiltAndroidTesting)"))
        assertTrue("Should include Hilt compiler for tests", 
    ...
    fun `debug dependencies are configured`() {
        ...
        assertTrue("Should include Compose UI tooling for debug", 
            content.contains("debugImplementation(libs.composeUiTooling)"))
        assertTrue("Should include Compose test manifest for debug", 
    ...
    fun `build script has valid kotlin dsl syntax`() {
        ...
        assertTrue("Should properly close all blocks", 
    ...
    fun `build script imports are valid`() {
        ...
        assertTrue("Should import JvmTarget", 
    ...
    fun `build script handles version catalog properly`() {
        ...
        assertTrue("Should use libs for version catalog", 
            content.contains("libs.versions"))
        assertTrue("Should use proper get() method", 
    ...
    fun `configuration blocks are not empty`() {
        ...
        assertFalse("Android block should not be empty", 
            content.contains("android {\\s*}".toRegex()))
        assertFalse("Dependencies block should not be empty", 
    ...
    fun `sdk versions are compatible`() {
        ...
        assertTrue("compileSdk should be >= targetSdk", 
            content.contains("compileSdk = 36") && content.contains("targetSdk = 34"))
        assertTrue("targetSdk should be >= minSdk", 
    ...
    fun `plugin order is correct for hilt and ksp`() {
        ...
        assertTrue("KSP must be applied before Hilt", 
    ...
    fun `build script includes performance optimizations`() {
        ...
        assertTrue("Should enable vector drawable support", 
            content.contains("useSupportLibrary = true"))
        assertTrue("Should have resource compression configuration", 
            content.contains("noCompress"))
        assertTrue("Should exclude unnecessary resources", 
    ...
    fun `proguard configuration is present for release`() {
        ...
        assertTrue("Should include proguard files in release build", 
            content.contains("proguardFiles"))
        assertTrue("Should use optimized proguard config", 
    ...
    fun `security dependencies are included`() {
        ...
        assertTrue("Should include Android security crypto", 
    ...
    fun `required build features are enabled`() {
        ...
        if (buildFeaturesMatch != null) {
            ...
            assertTrue("BuildConfig should be enabled in buildFeatures", 
            ...
            assertTrue("ViewBinding should be enabled in buildFeatures", 
    ...
    fun `android resources are properly configured`() {
        ...
        assertTrue("Should configure no-compress files", 
            content.contains("noCompress += listOf(\"proto\", \"json\")"))
        assertTrue("Should ignore assets pattern", 
    ...
    fun `coroutines dependencies are complete`() {
        ...
        assertTrue("Should include coroutines Android", 
            content.contains("implementation(libs.kotlinxCoroutinesAndroid)"))
        assertTrue("Should include coroutines core", 
    ...
    fun `external libraries are properly included`() {
        ...
        assertTrue("Should include Xposed API", 
    ...
    fun `build script contains no obvious syntax errors`() {
        ...
        assertFalse("Should not contain unmatched quotes", 
            content.count { it == '"' } % 2 != 0)
        assertFalse("Should not contain trailing commas in wrong places", 
    ...
    fun `version references use proper syntax`() {
        ...
        assertTrue("Version references should use proper syntax", 
    ...
    fun `dependencies are organized efficiently`() {
        ...
        assertTrue("Should use Compose BOM for version management", 
            content.contains("platform(libs.composeBom)"))
        assertTrue("Should use Firebase BOM for version management", 
